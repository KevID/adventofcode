...
#[AsCommand(name: 'app:2023:03:B')]
class Day03BCommand extends Command
{
    /**
     * @var array<string, int[]> Stores gear positions as keys and arrays of adjacent numbers as values.
     */
    private array $numbersAdjacentToGears = [];
    
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $io = new SymfonyStyle($input, $output);
        $io->title('Advent of Code - Day 03 - Challenge B');
        ...
        $io->newLine(2);
        $io->success('Sum of all of the calibration values: ' . $this->sumGearsRatio());
        $io->progressFinish();
    ...
    */
    private function processLine(array $lines, int $key, string $line): void
    {
        ...
        foreach ($numbers[0] as $number) {
            $positionGearAdjacent = $this->returnPositionGearAdjacent($lines, $key, $number[1], strlen($number[0]));
            if ($positionGearAdjacent !== null) {
                $this->numbersAdjacentToGears[$positionGearAdjacent][] = (int)$number[0];
            }
        }
    }
     ...
     * @return string|null
     */
    private function returnPositionGearAdjacent(array $lines, int $key, int $offset, int $length): ?string
    {
        for ($i = $key - 1; $i <= $key + 1; $i++) {
            if (array_key_exists($i, $lines)) {
                $positionInLine = $this->returnPositionAdjacentToGearInLine($lines[$i], $offset, $length);
                if ($positionInLine) {
    ...
    /**
     * Checks if a number in a line of text is adjacent to a gear *.
     ...
     * @return int|null Return position of the adjacent gear in the line if exist.
     */
    private function returnPositionAdjacentToGearInLine(string $line, int $offset, int $length): ?int
    {
        ...
        preg_match_all('/\*/', $characters, $gear, PREG_OFFSET_CAPTURE);
        
        if ($gear[0]) {
            return $offset + $offsetAdjustment + (int)$gear[0][0][1];
        }

        return null;
    }
    
    private function sumGearsRatio(): float
    {
        $sum = 0;
        foreach ($this->numbersAdjacentToGears as $numbers) {
            if(count($numbers) === 2) {
